...
const papersQ = (slug: string) =>
  queryOptions({
    queryKey: ["papers", "subject", slug, "exams"],
    queryFn: () => listPapers({ data: { subjectSlug: slug } }),
  });

export const Route = createFileRoute("/subjects/$slug/exams")({
...
function Page() {
  ...
  const online = papers.filter((p) => p.has_online_exam);
    ...
    <SiteLayout>
      <section className="bg-navy text-navy-foreground">
        <div className="mx-auto max-w-7xl px-4 py-8">
          ...
          <h1 className="text-2xl md:text-3xl font-bold">{subject.name} — Online Exams</h1>
          <p className="opacity-90 mt-2">{t("Online ලිවීමට available papers.", "Papers available to write online.")}</p>
        </div>
      </section>
      <section className="mx-auto max-w-7xl px-4 py-8">
        {online.length === 0 ? (
          <div className="text-center text-muted-foreground py-12">{t("තවම online ලිවීමට papers add කර නැත.", "No online papers have been added yet.")}</div>
        ) : (
          <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {online.map((p) => <PaperCard key={p.id} paper={p} />)}
          </div>
        )}
      </section>
    </SiteLayout>
